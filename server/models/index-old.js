...
// In production without database, skip Sequelize initialization
if (process.env.NODE_ENV === 'production' && !process.env.MYSQL_URL && !process.env.MYSQLHOST) {
  ...
  // Railway MySQL with individual env vars
  ...
  // Traditional MySQL configuration
  sequelize = new Sequelize(
...
let Creator, Design, DesignProduct, DesignVariant, DesignAnalytics;
...
const DesignProduct = sequelize.define('DesignProduct', {
  id: {
...
const DesignAnalytics = sequelize.define('DesignAnalytics', {
  id: {
    type: DataTypes.INTEGER,
    primaryKey: true,
    autoIncrement: true
...
// Define associations only if database is available
if (sequelize) {
...
Design.clearImageCache = async function(designId) {
  // This would interface with your canvas image generator
  // to clear cached images for a specific design
